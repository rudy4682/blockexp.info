...
function get_market_data(market, cb) {
  switch(market) {
    case 'alcurex':
      alcurex.get_data(settings.markets.coin, settings.markets.exchange, function(err, obj){
        return cb(err, obj);
      });
      break;
    case 'bittrex':
    ...
    case 'bleutrade':
      ...
      break;
    case 'btce':
      btce.get_data(settings.markets.coin.toLowerCase(), settings.markets.exchange.toLowerCase(), function(err, obj){
        return cb(err, obj);
      });
      break;
    case 'poloniex':
...
module.exports = {
  ...
  get_heavy: function(coin, cb) {
    ...
    get_distribution: function (richlist, stats, cb) {
        var distribution = {
            ...
            t_101_125: { percent: 0, total: 0 },
            t_126_150: { percent: 0, total: 0 },
            t_151_175: { percent: 0, total: 0 },
            t_176_200: { percent: 0, total: 0 },
        ...
        lib.syncLoop(richlist.balance.length, function (loop) {
            ...
            if (count <= 125 && count > 100) {
                distribution.t_101_125.percent = distribution.t_101_125.percent + percentage;
                distribution.t_101_125.total = distribution.t_101_125.total + (richlist.balance[i].balance / 100000000);
            }
            if (count <= 150 && count > 125) {
                distribution.t_126_150.percent = distribution.t_126_150.percent + percentage;
                distribution.t_126_150.total = distribution.t_126_150.total + (richlist.balance[i].balance / 100000000);
            }
            if (count <= 175 && count > 150) {
                distribution.t_151_175.percent = distribution.t_151_175.percent + percentage;
                distribution.t_151_175.total = distribution.t_151_175.total + (richlist.balance[i].balance / 100000000);
            }
            if (count <= 200 && count > 175) {
                distribution.t_176_200.percent = distribution.t_176_200.percent + percentage;
                distribution.t_176_200.total = distribution.t_176_200.total + (richlist.balance[i].balance / 100000000);
            }
            loop.next();
        }, function () {
            distribution.t_201plus.percent = parseFloat(100 - distribution.t_176_200.percent - distribution.t_151_175.percent - distribution.t_126_150.percent - distribution.t_101_125.percent - distribution.t_76_100.percent - distribution.t_51_75.percent - distribution.t_26_50.percent - distribution.t_1_25.percent).toFixed(2);
            distribution.t_201plus.total = parseFloat(distribution.supply - distribution.t_176_200.total - distribution.t_151_175.total - distribution.t_126_150.total - distribution.t_101_125.total - distribution.t_76_100.total - distribution.t_51_75.total - distribution.t_26_50.total - distribution.t_1_25.total).toFixed(8);
            ...
            distribution.t_101_125.percent = parseFloat(distribution.t_101_125.percent).toFixed(2);
            distribution.t_101_125.total = parseFloat(distribution.t_101_125.total).toFixed(8);
            distribution.t_126_150.percent = parseFloat(distribution.t_126_150.percent).toFixed(2);
            distribution.t_126_150.total = parseFloat(distribution.t_126_150.total).toFixed(8);
            distribution.t_151_175.percent = parseFloat(distribution.t_151_175.percent).toFixed(2);
            distribution.t_151_175.total = parseFloat(distribution.t_151_175.total).toFixed(8);
            distribution.t_176_200.percent = parseFloat(distribution.t_176_200.percent).toFixed(2);
            distribution.t_176_200.total = parseFloat(distribution.t_176_200.total).toFixed(8);
  ...
  update_db: function(coin, cb) {
    lib.get_blockcount( function (count) {
      ...
      lib.get_supply( function (supply){
                    Nodes.get_peerinfo(function (conninfo) {
